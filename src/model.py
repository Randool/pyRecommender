...
class MKR(object):
    ...
    def _build_low_layers(self, args):
        ...
        # embedding_lookup: 根据id的值选取tensor中不同维度的向量并进行拼接
        # [batch_size, dim]
        ...
        for _ in range(args.L):  # L: number of low layers, default 1
    ...
    def _build_high_layers(self, args):
        ...
        use_inner_product = True  # 向量内积
        if use_inner_product:
            # reduce_sum: 压缩求和，axis指定讲哪一维求和
            # [batch_size]
        ...
        # number of high layers, default 1
        for _ in range(args.H - 1):
        ...
        self.rmse = tf.reduce_mean(
            tf.sqrt(
                tf.reduce_sum(tf.square(self.tail_embeddings - self.tail_pred), axis=1)
                / args.dim
            )
        )
    ...
    def search(self, sess, feed_dict):
        """ 基于知识图谱的检索功能 """
        return sess.run([self.tail_pred], feed_dict)
